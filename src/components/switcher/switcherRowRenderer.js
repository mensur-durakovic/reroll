...
const SwitcherRowRenderer = React.memo((props) => {
    const {rowChoices, choiceHandler, lockChoices, isLastRow, rowLevel, calculateWinner} = props;
    ...
    const generateSwitchers = () => {
        ...
        for (let i = 0; i < rowChoicesCount; i += 2) {
            ...
            if (rowChoices.hasOwnProperty(i + 2) && !rowChoices.hasOwnProperty(i + 3)) {
        ...
        <section className="second-page-final">
            ...
            {!isDisabled && (
                <div className="second-page-final-button">
                    <ContinueButtonRenderer isLastRow={isLastRow} calculateWinner={calculateWinner} optionsDisabler={optionsDisabler}/>
                </div>
            )}
        </section>
    );
