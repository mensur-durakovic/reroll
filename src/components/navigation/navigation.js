...
const Navigation = React.memo((props) => {
  const { navChoiceHandler, navOptions } = props;
  const [animationDirection, setAnimationDirection] = useState("");

  const getResourceImage = (name) => {
    switch (name) {
      case NAVIGATION_ITEM_ALL:
        return AllActive;
      case NAVIGATION_ITEM_TANK_ONLY:
        return TankActive;
      case NAVIGATION_ITEM_HEALER_ONLY:
        return HealerActive;
      case NAVIGATION_ITEM_RANGED_DPS_ONLY:
        return RangedActive;
      case NAVIGATION_ITEM_MELEE_DPS_ONLY:
        return MeleeActive;
      default:
        return AllActive;
    }
  ...
  const handleAnimationDirection = (itemName) => {
    const currentActive = navOptions.findIndex((item) => item.isActive);
    const newActive = navOptions.findIndex((item) => item.name === itemName);
    setAnimationDirection(newActive > currentActive ? "left" : "right");
    navChoiceHandler(itemName);
  };

  const mappedNavItems = navOptions
    ? navOptions.map((item) => (
        <li key={`navigation-item-${Math.random()}`}>
          ...
          >
            ...
            <div className="navigation-logo-wrapper">
              ...
              ></div>
              <div
                className={`navigation-logo-icon-wrapper ${
                  !item.isActive ? "navigation-logo-icon-wrapper-gray" : ""
                }`}
              >
